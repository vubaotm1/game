...
(function(){
    ...
    Physics.body = Decorator('body', {
        ...
        init: function( options ){
            ...
            // properties
            this.fixedAngle = this.options.fixedAngle;
    ...
    return {
        ...
        collideBodies: function(bodyA, bodyB, normal, point, mtrans, contact){
            ...
            var fixedA = bodyA.fixed
                ,fixedB = bodyB.fixed,
                fixedAngleA = bodyA.fixedAngle,
                fixedAngleB = bodyB.fixedAngle
                ,scratch = Physics.scratchpad()
            ...
            if ( fixedA ){
                ...
                if(!fixedAngleB) bodyB.state.angular.vel -= impulse * invMoiB * rBreg;
                
            } else if ( fixedB ){

                // apply impulse
                bodyA.state.vel.vsub( n.mult( impulse * invMassA ) );
                if(!fixedAngleA) bodyA.state.angular.vel += impulse * invMoiA * rAreg;
            ...
            if ( cof && vreg ){
                ...
                if ( fixedA ){
                    ...
                    if(!fixedAngleB) bodyB.state.angular.vel -= impulse * invMoiB * rBproj;
                    
                } else if ( fixedB ){

                    // apply frictional impulse
                    bodyA.state.vel.vadd( perp.mult( impulse * invMassA ) );
                    if(!fixedAngleA) bodyA.state.angular.vel += impulse * invMoiA * rAproj;
